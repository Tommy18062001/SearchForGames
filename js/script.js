...
const fields =
  "fields=name, genres.name, category, cover.image_id, release_dates.human, release_dates.y, platforms.name, rating, url";
...
function mainRequest(value, field) {
  ...
  fetch(
    `https://cors-anywhere.herokuapp.com/${url}games?search=${value}&${field};limit=30;`,
    requestOptions
  )
...
function displayGame(gameData) {
  ...
  if (gameData.cover) {
    ...
    img.setAttribute(
      "src",
      `https://images.igdb.com/igdb/image/upload/t_cover_big/undefined.png`
    );
    img.setAttribute("alt", "cover");
  }
  ...
  if (gameData.platforms) {
    ...
    platform.textContent = "Platforms: N/A";
  }
  ...
  if (gameData.release_dates) {
    ...
    release.textContent = "Release date: N/A";
  }
  ...
  btn.addEventListener("click", () => {
    ...
    if (btn.dataset.sort === "date") {
      ...
      Array.from(document.querySelectorAll(".game[data-default]"))
        .sort(({ dataset: { default: a } }, { dataset: { default: b } }) =>
          a.localeCompare(b)
        )
        .forEach((item) => item.parentNode.appendChild(item));
    }
